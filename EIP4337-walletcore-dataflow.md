...
5. 权限验证
   ...
   这一步跟传统EOA差别比较大,而且暂时也没有已经共识的标准,我们做的钱包contract-bridge-SDK需要调研且考虑清楚那种认证方式相对更优,个人建议:可以在每个contract wallet 中实现一个类似ValidateSignature的函数用户传入EIP-712的签名信息,DAPP把这个签名信息提供调用合约验证,但是这里面有一个很大的风险,如果DAPP有后端服务器还好,如果没有后端服务器其实这个签名验证特别容易被钱包插件或者中间层劫持且伪造 
