...
+ Besides basic wallet feature supporting ERC-4337, Our POV will focus on [social recovery](https://vitalik.ca/general/2021/01/11/recovery.html).
+ We provide the ability of changing the control of your wallet(secret key) within your guardian's permission and without modifying Ethereum protocol. 
+ Conception architecture：
+ ![SoulWallet architecture](4337-product-architecture.png)
### Contract wallet
+ We need a wallet to realize the mission we talking above based on what [infinitism](https://github.com/eth-infinitism/account-abstraction) has done.
+ It has some components like this:
+ ![](main-flow2.png)
+ 
### Social recovery
+ The highlight, social recovery's draft flow chart:
+ ![social recovery](SR-flow.jpg) 
### Security center
+ We need a security center to enable guardians to recover the wallet for the users.
...
### Tech stack
+ After launching the POC , we will build a small community to get the feedback.
+ We will base on @Mask 's years technical accumulation to develop our product to be compatable with MetaMask.
+ Thanks for the Open Source spirit on @Mask and special thanks for @yisi @gaoyuan.

### Team
+ We will recruit new devs (fulltime first) to join us to build the SoulWallet.
+ We will build cooperations with other great teams like Astrox, to build AA and SoulWallet together.

### Venture
+ We'll start fund raising after POC launch to enable devs to work on this project full time;
+ Contact @zengjiajun_eth @jhfnetboy at twitter.

## Quick view of SoulWallet
+ No Seed / No Gas / Full Control / Social Recovery / Programmable 
